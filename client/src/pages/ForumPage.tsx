...
const CommentNode: React.FC<{ comment: any; postId: string; onReplyAdded: () => void }> = ({ comment, postId, onReplyAdded }) => {
    ...
    return (
        <div className="pl-2 sm:pl-4 border-l-2 border-gray-200 dark:border-gray-700 mt-2 sm:mt-3">
            <div className="flex items-start gap-1.5 sm:gap-2">
                <CornerDownRight className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-gray-400 shrink-0 mt-1" />
                <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-gray-700 dark:text-gray-300 mb-1 break-words">{comment.body}</p>
                    {user && (
                        <button onClick={() => setShowReply(!showReply)} className="text-xs text-primary-500 hover:text-primary-700 font-medium min-h-[32px] inline-flex items-center">
                            Reply
                        </button>
                    )}
                    {showReply && (
                        <div className="flex gap-2 mt-2">
                            <input
                                className="flex-1 text-base sm:text-sm px-3 py-2 border rounded-xl dark:bg-gray-800 dark:border-gray-700 dark:text-white outline-none focus:ring-1 focus:ring-primary-500 min-h-[40px]"
                                placeholder="Write a reply..."
                                value={replyText}
                                onChange={e => setReplyText(e.target.value)}
                                onKeyDown={e => e.key === 'Enter' && handleSubmitReply()}
                            />
                            <Button size="sm" onClick={handleSubmitReply} isLoading={submitting} className="min-h-[40px] px-3"><Send className="w-3.5 h-3.5" /></Button>
                        </div>
                    )}
        ...
        <div className="space-y-8">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-gray-500 text-sm sm:text-base">Share your story safely and anonymously.</p>
                </div>
                {!isCreating && (
                    user ? (
                        <Button onClick={() => setIsCreating(true)} className="w-full sm:w-auto justify-center min-h-[44px] gap-2">
                            <Plus className="w-5 h-5" /> New Post
                        </Button>
                    ) : (
                        <Button variant="outline" onClick={() => navigate('/auth')} className="w-full sm:w-auto justify-center min-h-[44px] text-sm">
                            Log in to Post
                        </Button>
                    )
                )}
            ...
            {isCreating && (
                <Card className="p-5 sm:p-6 border-2 border-primary-100 dark:border-primary-900 animate-fade-in-up">
                    <div className="flex justify-between items-center mb-4">
                        <h3 className="font-bold text-lg dark:text-white">Create New Post</h3>
                        <button onClick={() => setIsCreating(false)} className="p-2 text-gray-400 hover:text-gray-600 min-h-[44px] min-w-[44px] flex items-center justify-center"><X className="w-5 h-5" /></button>
                    </div>
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div className="grid md:grid-cols-2 gap-4">
                            <Input placeholder="Title" value={newPost.title} onChange={e => setNewPost({...newPost, title: e.target.value})} required />
                            <select className="w-full min-h-[44px] px-4 py-2.5 border rounded-xl bg-white dark:bg-gray-800 dark:text-white border-gray-300 dark:border-gray-600 text-base sm:text-sm" value={newPost.category} onChange={e => setNewPost({...newPost, category: e.target.value})}>
                        ...
                        <div className="flex flex-col-reverse sm:flex-row justify-end gap-2">
                            <Button type="button" variant="ghost" onClick={() => setIsCreating(false)} className="w-full sm:w-auto min-h-[44px]">Cancel</Button>
                            <Button type="submit" className="w-full sm:w-auto min-h-[44px]">Post Anonymously</Button>
                        </div>
                    </form>
                </Card>
            )}
            ...
            <div className="space-y-4">
                ...
                {posts.map(post => (
                    <Card key={post.id} className="p-5 sm:p-6 hover:shadow-md transition-shadow">
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-gray-100 dark:border-gray-800 pt-4">
                            <div className="flex items-center gap-3 sm:gap-4">
                                <button onClick={() => handleHug(post.id)} className="flex items-center gap-1.5 text-gray-500 hover:text-pink-500 transition-colors text-sm font-medium group min-h-[40px] px-1">
                                    <Heart className="w-4 h-4 group-hover:scale-110 transition-transform" /> {post.hugs} Hugs
                                </button>
                                <button onClick={() => toggleComments(post.id)} className="flex items-center gap-1.5 text-gray-500 hover:text-blue-500 transition-colors text-sm font-medium group min-h-[40px] px-1">
                                    <MessageCircle className="w-4 h-4 group-hover:scale-110 transition-transform" /> 
                                    <span>{expandedPost === post.id ? 'Close' : 'Replies'}</span>
                                </button>
                            </div>
                            <button
                                onClick={() => !post.isFlagged && handleReport(post.id)}
                                className={`flex items-center gap-1.5 text-xs group transition-colors min-h-[40px] px-2 ${post.isFlagged ? 'text-red-500 cursor-default' : 'text-gray-400 hover:text-red-500'}`}
                                disabled={post.isFlagged}
                            >
                                <Flag className={`w-3.5 h-3.5 ${!post.isFlagged && 'group-hover:scale-110 transition-transform'}`} fill={post.isFlagged ? "currentColor" : "none"} />
                            ...
                            <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-800">
                                ...
                                <div className="flex gap-2 mb-4">
                                    {user ? (
                                        <>
                                            <input
                                                className="flex-1 px-3 py-2 border rounded-xl dark:bg-gray-800 dark:border-gray-700 dark:text-white outline-none focus:ring-1 focus:ring-primary-500 text-base sm:text-sm min-h-[44px]"
                                            ...
                                            />
                                            <Button size="sm" onClick={() => handleSubmitTopLevelReply(post.id)} isLoading={replySubmitting === post.id} className="min-h-[44px] px-4">
                                                <Send className="w-4 h-4" />
                                            </Button>
                                        </>
                                    ) : (
                                        <button onClick={() => navigate('/auth')} className="w-full text-center py-2.5 bg-gray-100 dark:bg-gray-800 rounded-xl text-sm text-gray-500 hover:text-primary-600 hover:bg-primary-50 transition-colors min-h-[44px] flex items-center justify-center">
                                            Log in to reply
                                        </button>
                                    )}
                                </div>
