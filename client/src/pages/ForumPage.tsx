...
export const ForumPage = () => {
    ...
    React.useEffect(() => {
        StorageService.getForumPosts().then(setPosts);
    }, []);
    ...
    const submitPost = async (forceFlagged: boolean, forceTriggering: boolean) => {
        ...
        const updatedPosts = await StorageService.addForumPost(post);
    ...
    const handleReport = async (postId: string) => {
        if (confirm("Are you sure you want to report this post as inappropriate?")) {
            const updated = await StorageService.flagForumPost(postId);
            setPosts(updated);
        }
    };

    const handleHug = async (postId: string) => {
        const updated = await StorageService.hugForumPost(postId);
        setPosts(updated);
    };
