...
export const ChatPage = () => {
    ...
    useEffect(() => {
        if (!user) return;
        const loadChats = () => setConversations(StorageService.getConversations(user.id));
        loadChats();
    ...
    useEffect(() => {
        ...
        const loadMessages = () => {
            const msgs = StorageService.getMessages(selectedChatId);
            setMessages(msgs);
    ...
    const handleSendMessage = (e?: React.FormEvent) => {
        ...
        StorageService.sendMessage(selectedChatId, user, newMessage);
        setNewMessage('');
        setMessages(StorageService.getMessages(selectedChatId));
    };
             ...
             <Modal isOpen={showCrisisAlert} onClose={() => setShowCrisisAlert(false)} title="Safety Check">
                <div className="space-y-4">
                    ...
                    <div className="flex flex-col gap-3">
                        ...
                        <Button variant="secondary" onClick={() => { setShowCrisisAlert(false); StorageService.sendMessage(selectedChatId!, user!, newMessage); setNewMessage(''); }}>Send Anyway</Button>
                    </div>
                </div>
            </Modal>
