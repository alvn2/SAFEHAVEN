...
export const DeveloperDashboard = () => {
    ...
    const [apps, setApps] = useState(StorageService.getVolunteerApps());
    const [logs] = useState(StorageService.getAuditLogs());
    
    const handleAddArticle = () => {
        if(!newArticle.title) return;
        StorageService.saveArticle({ id: Date.now().toString(), ...newArticle, readTime: 5, image: 'https://placehold.co/600x400', type: 'article' });
