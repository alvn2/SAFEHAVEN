...
export const SeekerDashboard = () => {
    ...
    useEffect(() => {
        ...
        setEntries(StorageService.getJournalEntries(passphrase));
        const plan = StorageService.getSafetyPlan(user.id, passphrase);
    ...
    const handleSaveEntry = (isDraft: boolean = false) => {
        ...
        const updated = StorageService.upsertJournalEntry(newEntry, passphrase);
        setEntries(updated);
        if (isDraft) {
            setSaveStatus('saved');
        } else {
    ...
    const handleSavePlan = async () => {
        ...
        if (user) StorageService.saveSafetyPlan({ ...safetyPlan, userId: user.id }, passphrase);
    ...
    const handleOpenNukeModal = () => {
        if (user) {
            const recoveryPhrase = StorageService.getRecoveryPhrase(user, passphrase);
            if (recoveryPhrase) {
                const words = recoveryPhrase.split(' ');
                const idx = Math.floor(Math.random() * words.length);
                setChallengeIndex(idx + 1);
                setChallengeWord(words[idx]);
            }
        }
    ...
    const handleNukeData = () => {
        ...
        if (challengeWord && wordInput.trim().toLowerCase() !== challengeWord.toLowerCase()) {
            alert(`Incorrect word. Please enter Word #${challengeIndex}.`);
            return;
        }
        if (user) {
            StorageService.deleteAccount(user.id);
            logout();
